...
%  Sweep time (microseconds)    1
%  Sweep bandwidth (MHz)        
%  Maximum beat frequency (MHz) 
%  Sample rate (MHz)            
...
for m = 1:Nsweep
    ...
    sig = waveform();
    % Can Add Phase Noise here
    sig_Noise = awgn(sig,1,'measured','db'); % Snr db
    
    %   ALEX PART - BEGIN:
    txsig_orig = transmitter(sig_Noise);
    
    % Propagate the signal and reflect off the target
    tgt_pos(1) = 2;
    txsig = channel(txsig_orig,radar_pos,tgt_pos,radar_vel,tgt_vel);
    ...
    txsig_mixed = txsig + 0.001*txsig_orig;
    
    % Dechirp the received radar return
    txsig = receiver(txsig_mixed);
    
    % Can Add Phase Noise here
    txsig_Noise = awgn(txsig,1,'measured','db'); % Snr db
    
    dechirpsig = dechirp(txsig_Noise,sig_Noise);
    %FFT
...
breakpoint = 0; %place break point here
