...
tot_sweep_time  = 4e-3  % (s) long sweep times create large signal arrays (slow)

Phase_NoiseAndOffset    = [-90,100e3] %Noise and Offset 
SystemWhite_Noise       = [-60]       %Iq Noise floor
Circulator_Issolation   = [-40];      %-20dB issolation
...
%target_comm = phased.RadarTarget('Model','Swerling2','MeanRCS',rcs_comm,'PropagationSpeed',c,...
%   'OperatingFrequency',fc);
...
for m = 1:Nsweep
    ...
    %% Setup the TX signal
    %txsig =  transmitter(sig); MATLAB2016b syntax
...
combined = wholesig;
 ...
 pnoise = comm.PhaseNoise('Level',PhaseNoise,'FrequencyOffset',Offset, ...
     'SampleRate',2*Offset);
 %IQ_Data_noise = pnoise(IQ_Data); %2016B MATLAB only
